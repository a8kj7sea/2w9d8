...
public class GuiAltManager extends GuiScreen {
	...
	public void actionPerformed(GuiButton button) throws IOException {
		...
		case 0: {
			...
			if (!this.loginThread.getStatus()
					.equals((Object) ((Object) EnumChatFormatting.YELLOW) + "Attempting to log in")
					&& !this.loginThread.getStatus().equals((Object) ((Object) EnumChatFormatting.RED)
							+ "Do not hit back!" + (Object) ((Object) EnumChatFormatting.YELLOW) + " Logging in...")) {
				this.mc.displayGuiScreen(null);
				break;
			}
			this.loginThread.setStatus((Object) ((Object) EnumChatFormatting.RED) + "Failed to login! Please try again!"
					+ (Object) ((Object) EnumChatFormatting.YELLOW) + " Logging in...");
			break;
		}
		case 1: {
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		this.drawCenteredString(fontRendererObj, sb2.append(AltManager.registry.size()).append(" alts").toString(),
				width / 2, 10, -1);
		...
		for (Alt alt2 : AltManager.registry) {
			...
			if (alt2 == this.selectedAlt) {
				...
				Gui.drawRect((int) 52.0f, y2 - this.offset - 4, width - 52, y2 - this.offset + 20, -16777216);
			} else if (this.isMouseOverAlt(par1, par2, y2 - this.offset)) {
				Gui.drawRect((int) 52.0f, y2 - this.offset - 4, width - 52, y2 - this.offset + 20, -16777216);
			}
	...
	private boolean isMouseOverAlt(int x2, int y2, int y1) {
		if (x2 >= 52 && y2 >= y1 - 4 && x2 <= width - 52 && y2 <= y1 + 20 && x2 >= 0 && y2 >= 33 && x2 <= width
				&& y2 <= height - 50) {
			return true;
		}
		return false;
